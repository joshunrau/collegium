...
describe('Ephemeral commands', () => {
  ...
  it('/trace returns a turn’s full tool trace to the invoker alone (§8.3)', async () => {
  ...
  it('/queue reports pending depth and the oldest unprocessed post (§8.4)', async () => {
    ...
    await channels.main.runCommand(`/queue ${agents.mira.username}`);
  ...
  it('/triggers lists outstanding triggers for an agent (§8.4)', async () => {
    ...
    await channels.main.runCommand(`/triggers ${agents.mira.username}`);
...
describe('Posting commands', () => {
  ...
  it('/forget removes a post from agent context (§8.4)', async () => {
    const { channels, inference } = harness();
    ...
    await channels.main.runCommand(`/forget ${seed.id}`);
  ...
  it('/reset marks an episode boundary that context never reaches past (§3.8)', async () => {
    const { agents, channels, inference } = harness();
    ...
    await channels.main.runCommand(`/reset ${agents.mira.username}`);
    await channels.main.awaitPost({
...
describe('/stop', () => {
  ...
  it('ends current turns in the channel at the next iteration boundary (§7.5)', async () => {
    ...
    await blocked.arrived;
    await channels.main.runCommand('/stop');
    await channels.main.awaitPost({
...
describe('/kill', () => {
  ...
  it('ends current turns in the channel immediately and releases the lock (§7.5)', async () => {
    ...
    await blocked.arrived;
    await channels.main.runCommand('/kill');
    await channels.main.awaitPost({
...
describe('/resume', () => {
  ...
  it('clears a global halt, draining the work queued behind it (§7.4)', async () => {
    ...
    await channels.main.runCommand('/resume');
    await channels.main.awaitPost({
...
describe('/resume and the rolling window', () => {
  const harness = setupHarness(RESUME_SCENARIO);
...
describe('Intervention scope', () => {
  ...
  it('/stop and /kill apply to every agent in the issuing channel (§7.5)', async () => {
    const { channels, inference } = harness();
  ...
  it('/stop and /kill leave the queue intact so pending work drains into the next turn (§7.5)', async () => {
    const { channels, inference } = harness();
  ...
  it('accepts /stop and /kill from any human in the channel (§7.5)', async () => {
    const { channels, inference } = harness();
    ...
    await blocked.arrived;
    await channels.main.runCommandAs(human, '/kill');
    await channels.main.awaitPost({
