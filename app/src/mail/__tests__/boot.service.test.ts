...
describe('MailBootService', () => {
  ...
  it('should pass a mailbox whose channel, membership, and credentials all hold', async () => {
    await expect(mailBootService.assertReadyAndAnnounceOutages()).resolves.toBeUndefined();
    expect(probe).toHaveBeenCalledTimes(1);
  });

  it('should refuse a DM announcement channel by name', async () => {
    isDirectMessageChannel.mockResolvedValue(Result.ok(true));
    await expect(mailBootService.assertReadyAndAnnounceOutages()).rejects.toThrow('direct message');
  });

  it('should refuse a channel the agent is not a member of', async () => {
    rosterService.isAgentIn.mockReturnValue(false);
    await expect(mailBootService.assertReadyAndAnnounceOutages()).rejects.toThrow('not a member');
  });

  it('should refuse a mailbox whose credentials are refused, naming the failure', async () => {
    probe.mockResolvedValue(Result.err({ kind: 'auth', message: 'the secret expired' }));
    await expect(mailBootService.assertReadyAndAnnounceOutages()).rejects.toThrow('the secret expired');
  });

  it('should boot with an unreachable mailbox, announcing the outage rather than refusing', async () => {
    probe.mockResolvedValue(Result.err({ kind: 'provider-unavailable', message: 'the socket disconnected' }));
    await expect(mailBootService.assertReadyAndAnnounceOutages()).resolves.toBeUndefined();
    expect(mailOutageService.announceUnreachable).toHaveBeenCalledWith(
      expect.objectContaining({ agentUsername: 'tess' }),
