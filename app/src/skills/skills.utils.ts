...
import { $SkillFrontmatter } from '@collegium/core/skills';
...
/**
 * A plugin's declared documents, read here rather than by the plugin loader: one module owns
 * reading a skill off disk (§9). The refusal names the plugin, because that is what an operator
 * must go and fix (§3.14).
 */
export function loadPluginSkillLibrary(source: PluginSkillSource): { [name: string]: Skill } {
  try {
    ...
    throw new Error(`plugin "${source.namespace}" declares a skill document that could not be loaded`, {
      cause: error
    });
  }
}
