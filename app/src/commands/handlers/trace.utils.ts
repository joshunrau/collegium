...
function renderStartedLine({ formatDate, turn }: TraceInput): string {
  const activation =
    turn.activationKind === null
      ? ''
      : `, by ${turn.activationKind} (${ACTIVATION_DESCRIPTIONS[turn.activationKind]})`;
...
function renderContextLine({ formatDate, turn }: TraceInput): string[] {
  ...
  const reach =
    turn.windowOldestAt === null ? 'an empty window' : `a window${size} reaching back to ${formatDate(turn.windowOldestAt)}`;
