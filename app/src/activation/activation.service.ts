...
export class ActivationService {
  ...
  private async drainQueue(profile: AgentProfile, channelId: string): Promise<void> {
    ...
    if (!(await this.haltService.admitTurnStart())) {
      this.loggingService.warn(
        `left the queue for "${profile.username}" in ${channelId} standing: the turn ceiling refused admission — /resume will drain it`
      );
      lock.release();
      return;
    }
  ...
  private async runTurn(
    ...
    try {
      ...
      this.loggingService.error(
        new Error(
          `a turn for "${profile.username}" threw past the runner — its queue and triggers stand until /resume or the next post`,
          { cause: error }
        )
      );
    } finally {
