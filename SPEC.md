...
- **A body is read back with its age and its written-at date** above the text, so a model reasons about staleness from the age and about provenance from the date. Neither is in the listing, so the prompt does not change as entries grow older.
