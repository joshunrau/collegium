...
**Browsing.** The `browser` tool drives a real rendered browser (Camoufox) in a turn-scoped session: navigate, click, and fill against element refs from the latest snapshot, each action returning the page as markdown. It is ungated as a read instrument. The per-agent grant decides who may browse at all, the status post traces every action live, and a per-action or per-session approval would stall autonomous turns while approving only an entry URL — never the later actions that could transmit. The tool's description forbids submitting forms that transmit information and entering credentials; the residual risk that a model does so anyway is accepted rather than hidden. Sessions are fresh anonymous contexts disposed at turn end — no cookies persist, no logins exist. robots.txt is not consulted: these are agent-driven reads at human pace, not crawling. Non-HTML resources (PDFs) are a typed refusal.
...
There is no threading. All posts are channel-level, so **context is pure recency**: no structural marker indicates where one piece of work ended and the next began. `/reset {agent}` provides a manual episode boundary; context never reaches back past the most recent one.
...
`POST /decisions`, `POST /commands`, and `POST /triggers` carry no authentication, by decision. Anyone who can reach `APP_PORT` can approve an agent action, issue `/kill`, or inject a trigger. The port must not be publicly routable — bind the loopback or a private interface and let Mattermost reach it over that path.
...
**The rolling window is durable; the halt flag is not.** Turn starts are counted from the store, from a persisted watermark that `/resume` advances, bounded to the trailing hour. The halt not surviving a restart is acceptable only because a restart breaks the loop that raised it; a crash-looping instance granting itself a fresh allowance every boot, with nobody deciding anything, would instead make the framework-wide number untrue. The ceiling is therefore re-evaluated at the first turn start after boot, and re-raises the halt if the window is still full.

### **7.5 Manual Intervention**

Both commands are **channel-scoped and apply to all agents in that channel.**

`/stop` aborts current turns at the next iteration boundary. The honest guarantee is _no further tool calls_, not _nothing happened_.

`/kill` abandons current turns immediately: the turn record is closed and the channel lock released. A tool already in flight may still complete and its side effect may still land — `/kill` is for a wedged process, and it accepts that ambiguity in exchange for immediacy.
...
Every memory write — and any eviction it causes — emits a disclosure line here (§3.6), the untruncated content of which can be viewed with `/trace`.
...
The complete tool trace — every call, arguments, and results — is retrievable via `/trace {post-id}`. **The response is ephemeral, visible only to the invoker**, because trace output contains file contents and email bodies verbatim, and everyone in a channel can also approve agents.
...
- **`/trace {post-id}`** — full tool trace for a turn. Ephemeral.
- **`/forget {post-id}`** — remove a post from agent context. Posts.
- **`/reset {agent}`** — mark an episode boundary. Posts.
- **`/stop`** — abort current turns in this channel at the next boundary. Posts.
- **`/kill`** — abandon current turns in this channel immediately. Posts.
- **`/resume`** — clear a global halt.
- **`/queue {agent}`** — show pending depth and the oldest unprocessed post. Ephemeral.
- **`/triggers {agent}`** — list outstanding triggers. Ephemeral.
- **memory review / delete** — inspect and prune an agent's memories. Ephemeral.

**The framework registers its own command surface at boot.** Each trigger above is reconciled against Mattermost. **Ownership is the creating account** — not the trigger word, and not the URL. A command this app created is its own to correct or remove; a command created by anyone else is never touched. An owned command is corrected when its URL has drifted, which is the redeploy case this exists for, and removed when its name has left the list above. A name in the list already held by a command the app does not own is a collision it cannot resolve, since Mattermost trigger words are unique per team: boot refuses, naming the command and its owner.

_Why not the URL:_ a command whose URL drifted is precisely the one needing correction, and URL-ownership would read it as foreign. _Why not the name:_ name-ownership would seize a `/stop` that legitimately belongs to another integration.
