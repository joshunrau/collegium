...
**The boundary is structural.** A plugin's sole import is `@collegium/sdk`: its tools are typed by the toolset they sit in — settings, storage, and the turn arrive as the declared context — and they reach the framework only through what that context hands them. The compiler enforces this rather than the spec asserting it: that one specifier resolves to the framework's own copy, so there is exactly one `zod` in the process by construction; `node:` builtins pass; and every other bare specifier is a boot refusal naming the plugin, the specifier, and the file that imported it. What the SDK does not export, a plugin cannot touch; the framework refactors freely behind it.
