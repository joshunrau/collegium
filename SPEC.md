...
**Browsing.** The `web` toolset drives a real rendered browser in a turn-scoped session: `web::navigate`, `web::click`, `web::fill`, and `web::hover` act against element refs from the latest snapshot, each action returning the page as markdown, in which an image is its alt text alone: its address is a read the model cannot make. A ref the page hides — a submenu that opens on hover — is marked `hidden`, and an action attempted on it is a typed refusal naming `web::hover`. A tab the page opens for itself is closed and reported with its address, so the session never moves onto a page nothing asked for. **It may submit forms and sign in where the task calls for it** — a great deal of the open web is unreachable otherwise.
