...
Every tool states its gate: `approval` is required, and a tool whose `approval` is a function always stops for a human, who sees the full payload before it runs, while one declaring `approval: null` never gates. Omitting the key refuses the plugin at boot. The channel and the trace disclose both, line by line. `execute` returns the text the model reads, and raises the two failures a tool controls through `err`: `invalidArguments` continues the turn, `unresolved` ends it as an unconfirmed side effect.
