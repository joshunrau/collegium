  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        intro:
          '`.env` also sets `MATTERMOST_PORT` and `POSTGRES_PASSWORD`, which `docker-compose.yaml` reads and the app never sees; the `APP_PORT` and `CONFIG_PATH` there are the host-side values Compose maps into the container.'
      }
    ],
    title: 'Environment'
  }
