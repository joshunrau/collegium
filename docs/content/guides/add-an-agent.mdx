...
- **Channels.** Set per-channel triggering in `config.json` under `mattermost.channels`, keyed by handle: `{ "support": { "triggeringMode": "respond-to-all" } }`. `mention-required` is the default. `respond-to-all` makes every human post start a turn and admits at most one agent in that channel. DMs behave as respond-to-all.
