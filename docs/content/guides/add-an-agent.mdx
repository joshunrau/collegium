...
- `skills` (optional) lists the skills Theo can load, by name for a framework skill or `plugin::name` for one a plugin ships. The two core skills are in every agent's manifest without being listed. A skill that names the tools its procedure calls refuses the boot for an agent granted the skill without every one of them, so grant the tools first.
