...
The directory name must be lowercase and dashed (`saving-contacts`), just as a tool's filename must be lowercase `snake_case`. `SKILL.md` and every `references/<name>.md` carry `title` and `description` frontmatter above a markdown body.
