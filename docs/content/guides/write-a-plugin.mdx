...
- `@collegium/sdk` and `zod` are the only entries in `dependencies`. Put `typescript`, `vitest`, and other tooling in `devDependencies`.
- Each declared range matches the version the deployment carries.
...
The loader reads direct children of `src/tools/` only, and refuses a file it cannot load as a tool. Keep helpers elsewhere under `src/` and import them by relative path. A subdirectory such as `src/tools/__tests__/` is yours; the loader ignores it.
...
To ship skills, add markdown files under `src/skills/`. Each `src/skills/<name>.md` becomes the grant `contacts::<name>`. See the in-repo `bookmark` plugin for an example.
