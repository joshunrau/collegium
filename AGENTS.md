...
- Whenever making any code changes, run `pnpm lint` and `pnpm test` from the repo root and fix failures before declaring the task done. Also run `pnpm test:e2e` unless the diff touches only markdown, unit tests, `docs/`, `benchmark/`, `scripts/`, `.agents/`, `.claude/`, or `.github/`.
...
app/src/
  ...
  utils/
  web/            the browser: turn-scoped Camoufox sessions, ref-stamped snapshots, page-to-markdown; and the plain HTTP fetch (a PDF read by its text layer, in a memory-capped child process) and provider-backed search beside it
  workspace/      the workspace directory and its confinement check, and the workspace toolset
```
